...
int rlConnect(int theSocket, const char* theAddress, short port)
{
  int theStatus = 0;
  struct sockaddr_in theDestination;
  theDestination.sin_family = AF_INET;
  theDestination.sin_port = htons(port);
...
int rlListen(int theSocket, short port)
{
